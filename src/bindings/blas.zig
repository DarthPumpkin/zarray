...
pub fn rot_real(
    ...
    const x_blas = Blas1dMut(Scalar).init(AxisI, x_na);
    const y_blas = Blas1dMut(Scalar).init(AxisI, y_na);
    f(
        x_blas.len,
        x_blas.ptr,
        x_blas.inc,
...
/// Only the triangle of `A` specified by `A.triangle` is read.
/// `x`'s axis must match one axis of `A.data` and `y`'s axis must match the other.
/// The scalars `alpha` and `beta` are optional and default to 1.
pub fn hemv(
    comptime Scalar: type,
    comptime AxisA: type,
    ...
    _ = named_index.resolveDimensions(.{ A.data.idx.shape, x.idx.shape, y.idx.shape }) catch
...
pub fn trmv(
    ...
    _ = named_index.resolveDimensions(.{ A.data.idx.shape, x.idx.shape }) catch
...
/// `A.data` is a 2D array with a band axis (size `K + 1`) and a vector axis
/// matching `x`'s axis (size N). `K` is inferred from the band axis size.
/// `A.triangle` selects the stored triangle (second axis → upper, first axis → lower).
/// The scalars `alpha` and `beta` are optional and default to 1.
///
/// **Storage requirement**: The band axis of `A` must be contiguous (stride 1)
...
pub fn sbmv(
    ...
    const A_band = BlasBand(Scalar, AxisA, AxisX).init(A.data);
    const x_blas = Blas1d(Scalar).init(AxisX, x);
...
/// Only the triangle of `A` specified by `A.triangle` is read and written.
/// `x`'s axis must match one axis of `A.data`.
/// The scalar `alpha` is optional and defaults to 1.
pub fn syr(
    comptime Scalar: type,
    comptime AxisA: type,
    comptime AxisX: type,
    A: BlasTriangularMut(Scalar, AxisA),
    x: NamedArrayConst(AxisX, Scalar),
    scalars: struct { alpha: Scalar = one(Scalar) },
) void {
    comptime assertMatchingAxis(AxisA, AxisX);
    ...
    const A_blas = Blas2dMut(Scalar).fromNamedArray(AxisA, A.data, 0);
    assert(A_blas.rows == A_blas.cols);
...
pub fn her(
    ...
    comptime AxisA: type,
    comptime AxisX: type,
    A: BlasTriangularMut(Complex(RealScalar), AxisA),
    x: NamedArrayConst(AxisX, Complex(RealScalar)),
    scalars: struct { alpha: RealScalar = 1.0 },
) void {
    const Scalar = Complex(RealScalar);
...
test "dot internal_double_precision" {
    ...
    const actual = dot(T, I, x, y, .{ .internal_double_precision = true });
    try std.testing.expectApproxEqAbs(
        expected,
        actual,
        math.floatEpsAt(T, expected),
...
test "rotg_real" {
    ...
    const rotation = rotg_real(T, &a, &b);
...
test "rot_real" {
    ...
    const rot = GivensRotationReal(T){
        .c = math.cos(theta),
        .s = math.sin(theta),
    };
    var points_buf = [_]T{
    ...
    const points = NamedArray(IJ, T).init(.initContiguous(.{ .i = 2, .j = 2 }), &points_buf);
    const expected_points = &[_]T{
        -math.sqrt(2.0) / 2.0, math.sqrt(2.0) / 2.0,
        2.0,                   0.0,
    };

    rot_real(T, rot, points);
...
test "rotmg" {
    ...
    const rotation = rotmg(T, &d1, &d2, &a, b);
...
test "rotm" {
    ...
    const rot1 = ModifiedGivensRotation(T){ .data = p1 };
    const rot2 = ModifiedGivensRotation(T){ .data = p2 };
    const rot3 = ModifiedGivensRotation(T){ .data = p3 };
    const rot4 = ModifiedGivensRotation(T){ .data = p4 };
    ...
    const idx = NamedIndex(IJ).initContiguous(.{ .i = 2, .j = 2 }).stride(.{ .i = -1 });
    const points1 = NamedArray(IJ, T).init(idx, &points_buf1);
    const points2 = NamedArray(IJ, T).init(idx, &points_buf2);
    const points3 = NamedArray(IJ, T).init(idx, &points_buf3);
    const points4 = NamedArray(IJ, T).init(idx, &points_buf4);
...
test "ModifiedGivensRotation init/matrix/flag round-trip" {
    inline for (.{ f32, f64 }) |T| {
        const MGR = ModifiedGivensRotation(T);
...
test "ModifiedGivensRotation applied via rotm matches reference for each flag" {
    ...
    for (forms) |m| {
        ...
        const points = NamedArray(IJ, T).init(idx, &pts);
        rotm(T, rot, points);
...
test "gemv real padded row-major" {
    ...
    gemv(T, MK, K, M, A, x, y, .{ .alpha = 1.0, .beta = 0.0 });
...
test "symv padded row-major" {
    ...
    symv(T, MK, K, M, .{ .triangle = .k, .data = A }, x, y, .{ .alpha = 1.0, .beta = 0.0 });
...
test "gemv real nontrivial scalars and strides" {
    ...
    gemv(T, MK, K, M, A, x, y, .{ .alpha = 2.0, .beta = -1.0 });
...
test "gemv real column-major matrix" {
    ...
    const A = NamedArrayConst(IJ_local, T).init(.{
        .shape = .{ .i = 2, .j = 2 },
    ...
    gemv(T, IJ_local, J, I, A, x, y, .{ .alpha = 1.0, .beta = 0.0 });
...
test "gemv real 1x1 matrix" {
    ...
    gemv(T, MK, K, M, A, x, y, .{ .alpha = 3.0, .beta = 2.0 });
...
test "hemv upper (triangle = second axis)" {
    ...
    hemv(T, MK, K, M, .{ .triangle = .k, .data = A }, x, y, .{
        .alpha = .{ .re = 1, .im = 0 },
        .beta = .{ .re = 0, .im = 0 },
    });
...
test "hemv lower (triangle = first axis)" {
    ...
    hemv(T, MK, K, M, .{ .triangle = .m, .data = A }, x, y, .{
        .alpha = .{ .re = 1, .im = 0 },
        .beta = .{ .re = 0, .im = 0 },
    });
...
test "hemv nontrivial scalars and strides" {
    ...
    hemv(T, AB, B, A_, .{ .triangle = .b, .data = A }, x, y, .{
        .alpha = .{ .re = 1, .im = 1 },
        .beta = .{ .re = 2, .im = 0 },
    });
...
test "hemv column-major matrix" {
    ...
    hemv(T, IJ_local, J, I, .{ .triangle = .j, .data = A }, x, y, .{
        .alpha = .{ .re = 1, .im = 0 },
        .beta = .{ .re = 0, .im = 0 },
    });
...
test "symv lower (triangle = first axis)" {
    ...
    symv(T, MK, K, M, .{ .triangle = .m, .data = A }, x, y, .{ .alpha = 1.0, .beta = 0.0 });
...
test "symv nontrivial scalars and strides" {
    ...
    symv(T, AB, B, A_, .{ .triangle = .b, .data = A }, x, y, .{ .alpha = 2.5, .beta = -1.0 });
...
test "symv column-major matrix" {
    ...
    symv(T, IJ_local, J, I, .{ .triangle = .j, .data = A }, x, y, .{ .alpha = 1.0, .beta = 0.0 });
...
test "trmv real" {
    ...
    //   x[1] = 0*1 + 7*2 + 11*3 = 47
    //   x[2] = 0*1 + 0*2 + 13*3 = 39
    trmv(T, MK, K, .{ .triangle = .k, .data = A }, .non_unit, x);
...
test "trmv real unit diagonal" {
    ...
    //   x[1] = 4*1 + 1*2 + 0*3 = 6
    //   x[2] = 5*1 + 6*2 + 1*3 = 20
    trmv(T, MK, K, .{ .triangle = .m, .data = A }, .unit, x);
...
test "trmv nontrivial strides" {
    ...
    trmv(T, AB, B, .{ .triangle = .a, .data = A }, .non_unit, x);
...
test "trmv column-major matrix" {
    ...
    trmv(T, IJ_local, J, .{ .triangle = .j, .data = A }, .non_unit, x);
...
test "trsv real" {
    ...
    trsv(T, MK, K, .{ .triangle = .k, .data = A }, .non_unit, x);
...
test "trsv real unit diagonal" {
    ...
    trsv(T, MK, K, .{ .triangle = .m, .data = A }, .unit, x);
...
test "trsv nontrivial strides" {
    ...
    trsv(T, AB, B, .{ .triangle = .a, .data = A }, .non_unit, x);
...
test "trsv column-major matrix" {
    ...
    trsv(T, IJ_local, J, .{ .triangle = .j, .data = A }, .non_unit, x);
...
test "ger column-major matrix" {
    ...
    var a_buf = [_]T{ 0, 0, 0, 0, 0, 0 };
    const A = NamedArray(IJ_local, T).init(.{
        .shape = .{ .i = 2, .j = 3 },
        .strides = .{ .i = 1, .j = 2 },
    }, &a_buf);
...
test "syr real (triangle = second axis)" {
    ...
    //                                     [2,2] = 9+9=18
    syr(T, MK, K, .{ .triangle = .k, .data = A }, x, .{});
...
test "syr real nontrivial strides" {
    ...
    //            = [[8,_],[4,2]] + [[3,_],[1,7]]
    //            = [[11,_],[5,9]]
    syr(T, AB, B, .{ .triangle = .a, .data = A }, x, .{ .alpha = 2.0 });
...
test "syr column-major matrix" {
    ...
    syr(T, IJ_local, J, .{ .triangle = .i, .data = A }, x, .{});
...
test "her complex (triangle = second axis)" {
    ...
    //            = [[3+0i, 4+3i], [_, 7+0i]]
    her(f64, MK, K, .{ .triangle = .k, .data = A }, x, .{});
...
test "her complex nontrivial strides" {
    ...
    //            = [[7+0i, _], [3-3i, 7+0i]]
    her(f32, AB, A_, .{ .triangle = .a, .data = A }, x, .{ .alpha = 2.0 });
...
test "her column-major matrix" {
    ...
    her(f64, IJ_local, J, .{ .triangle = .i, .data = A }, x, .{});
...
test "syr2 real (triangle = second axis)" {
    ...
    syr2(T, MK, M, K, .{ .triangle = .k, .data = A }, x, y, .{});
...
test "syr2 real nontrivial strides" {
    ...
    //            = [[24, _], [22, 16]] + [[3, _], [1, 7]]
    //            = [[27, _], [23, 23]]
    syr2(T, AB, B, A_, .{ .triangle = .a, .data = A }, x, y, .{ .alpha = 2.0 });
...
test "syr2 column-major matrix" {
    ...
    syr2(T, IJ_local, I, J, .{ .triangle = .i, .data = A }, x, y, .{});
...
test "her2 complex (triangle = second axis)" {
    ...
    //            = [[3+0i, 2+5i], [_, 7+0i]]
    her2(T, MK, M, K, .{ .triangle = .k, .data = A }, x, y, .{});
...
test "her2 complex nontrivial strides" {
    ...
    //            = [[3+0i, _], [4+i, 9+0i]]
    her2(T, AB, B, A_, .{ .triangle = .a, .data = A }, x, y, .{ .alpha = .{ .re = 2, .im = 1 } });
...
test "gbmv real" {
    ...
    gbmv(T, BK, K, M, BlasBand(T, BK, K).init(AB), x, y, .{ .alpha = 1.0, .beta = 0.0 }, .{ .kl = 1, .ku = 1 });
...
test "sbmv real upper" {
    ...
    sbmv(T, BK, K, .{ .triangle = .k, .data = AB }, x, y, .{ .alpha = 1.0, .beta = 0.0 });
...
test "hbmv complex upper" {
    ...
    hbmv(T, BK, K, .{ .triangle = .k, .data = AB }, x, y, .{
        .alpha = .{ .re = 1, .im = 0 },
        .beta = .{ .re = 0, .im = 0 },
    });
...
test "tbmv real upper" {
    ...
    // triangle = .k (second axis) → Upper
    tbmv(T, BK, K, .{ .triangle = .k, .data = AB }, .non_unit, x);
...
test "tbsv real upper" {
    ...
    tbsv(T, BK, K, .{ .triangle = .k, .data = AB }, .non_unit, x);
...
test "her2 column-major matrix" {
    ...
    her2(T, IJ_local, I, J, .{ .triangle = .i, .data = A }, x, y, .{});
...
test "spmv real upper" {
    ...
    // triangle = .k (second axis) → Upper
    spmv(T, MK, K, M, .{ .triangle = .k, .major = .k, .data = &ap }, x, y, .{ .alpha = 1.0, .beta = 0.0 });
...
test "spmv real lower" {
    ...
    spmv(T, MK, K, M, .{ .triangle = .m, .major = .k, .data = &ap }, x, y, .{ .alpha = 1.0, .beta = 0.0 });
...
test "spmv real upper row-major" {
    ...
    spmv(T, MK, K, M, .{ .triangle = .k, .major = .m, .data = &ap }, x, y, .{ .alpha = 1.0, .beta = 0.0 });
...
test "spmv real nontrivial scalars and strides" {
    ...
    spmv(T, MK, K, M, .{ .triangle = .k, .major = .k, .data = &ap }, x, y, .{ .alpha = 2.0, .beta = 1.0 });
...
test "hpmv complex upper" {
    ...
    hpmv(T, MK, K, M, .{ .triangle = .k, .major = .k, .data = &ap }, x, y, .{
        .alpha = .{ .re = 1, .im = 0 },
        .beta = .{ .re = 0, .im = 0 },
    });
...
test "hpmv complex lower" {
    ...
    // triangle = .m (first axis) → Lower
    hpmv(T, MK, K, M, .{ .triangle = .m, .major = .k, .data = &ap }, x, y, .{
        .alpha = .{ .re = 1, .im = 0 },
        .beta = .{ .re = 0, .im = 0 },
    });
...
test "hpmv complex upper row-major" {
    ...
    // triangle = .k (upper), major = .m (first axis → row-major)
    hpmv(T, MK, K, M, .{ .triangle = .k, .major = .m, .data = &ap }, x, y, .{
        .alpha = .{ .re = 1, .im = 0 },
        .beta = .{ .re = 0, .im = 0 },
    });
...
test "tpmv real upper" {
    ...
    // triangle = .k (second axis) → Upper
    tpmv(T, MK, K, .{ .triangle = .k, .major = .k, .data = &ap }, .non_unit, x);
...
test "tpmv real lower" {
    ...
    // triangle = .m (first axis) → Lower
    tpmv(T, MK, K, .{ .triangle = .m, .major = .k, .data = &ap }, .non_unit, x);
...
test "tpmv real unit diagonal" {
    ...
    tpmv(T, MK, K, .{ .triangle = .k, .major = .k, .data = &ap }, .unit, x);
...
test "tpmv nontrivial strides" {
    ...
    tpmv(T, AB, B, .{ .triangle = .b, .major = .b, .data = &ap }, .non_unit, x);
...
test "tpsv real upper" {
    ...
    tpsv(T, MK, K, .{ .triangle = .k, .major = .k, .data = &ap }, .non_unit, x);
...
test "tpsv real lower" {
    ...
    tpsv(T, MK, K, .{ .triangle = .m, .major = .k, .data = &ap }, .non_unit, x);
...
test "tpsv real unit diagonal" {
    ...
    tpsv(T, MK, K, .{ .triangle = .k, .major = .k, .data = &ap }, .unit, x);
...
test "tpsv nontrivial strides" {
    ...
    tpsv(T, AB, B, .{ .triangle = .b, .major = .b, .data = &ap }, .non_unit, x);
...
test "spr real upper" {
    ...
    spr(T, MK, K, .{ .triangle = .k, .major = .k, .data = &ap }, x, .{});
...
test "spr real lower" {
    ...
    // triangle = .m (first axis) → Lower
    spr(T, MK, K, .{ .triangle = .m, .major = .k, .data = &ap }, x, .{});
...
test "spr real upper row-major" {
    ...
    // triangle = .k (upper), major = .m (first axis → row-major)
    spr(T, MK, K, .{ .triangle = .k, .major = .m, .data = &ap }, x, .{});
...
test "spr nontrivial alpha and strides" {
    ...
    spr(T, MK, K, .{ .triangle = .k, .major = .k, .data = &ap }, x, .{ .alpha = 2.0 });
...
test "hpr complex upper" {
    ...
    hpr(f64, MK, K, .{ .triangle = .k, .major = .k, .data = &ap }, x, .{});
...
test "hpr complex lower" {
    ...
    // triangle = .m (first axis) → Lower
    hpr(f64, MK, K, .{ .triangle = .m, .major = .k, .data = &ap }, x, .{});
...
test "hpr nontrivial alpha and strides" {
    ...
    hpr(f32, MK, K, .{ .triangle = .k, .major = .k, .data = &ap }, x, .{ .alpha = 2.0 });
...
test "spr2 real upper" {
    ...
    spr2(T, MK, K, M, .{ .triangle = .k, .major = .k, .data = &ap }, x, y, .{});
...
test "spr2 real lower" {
    ...
    // triangle = .m (first axis) → Lower
    spr2(T, MK, K, M, .{ .triangle = .m, .major = .k, .data = &ap }, x, y, .{});
...
test "spr2 nontrivial alpha and strides" {
    ...
    spr2(T, MK, K, M, .{ .triangle = .k, .major = .k, .data = &ap }, x, y, .{ .alpha = 2.0 });
...
test "hpr2 complex upper" {
    ...
    hpr2(T, MK, M, K, .{ .triangle = .k, .major = .k, .data = &ap }, x, y, .{});
...
test "hpr2 complex lower" {
    ...
    // triangle = .m (first axis) → Lower
    hpr2(T, MK, M, K, .{ .triangle = .m, .major = .k, .data = &ap }, x, y, .{});
...
test "hpr2 nontrivial alpha and strides" {
    ...
    hpr2(T, MK, M, K, .{ .triangle = .k, .major = .k, .data = &ap }, x, y, .{ .alpha = .{ .re = 2, .im = 1 } });
...
test "gemm real 3x3" {
    ...
    // C = 1*A*I + 0*C = A
    gemm(T, MK, KN, MN, A, B, C, .{ .alpha = 1.0, .beta = 0.0 });
...
test "gemm real nontrivial scalars" {
    ...
    // 3*C = [[3, 3], [3, 3]]
    // result = [[41, 47], [89, 103]]
    gemm(T, MK, KN, MN, A, B, C, .{ .alpha = 2.0, .beta = 3.0 });
...
test "gemm real 1x1" {
    ...
    // C = 2*3*4 + 5*1 = 24 + 5 = 29
    gemm(T, MK, KN, MN, A, B, C, .{ .alpha = 2.0, .beta = 5.0 });
...
test "symm real left (triangle = second axis)" {
    ...
    symm(T, MK, KN, MN, .{ .triangle = .k, .data = A }, B, C, .{ .alpha = 1.0, .beta = 0.0 });
...
test "symm real nontrivial scalars" {
    ...
    symm(T, MK, KN, MN, .{ .triangle = .k, .data = A }, B, C, .{ .alpha = 2.0, .beta = 3.0 });
...
test "hemm complex left (triangle = second axis)" {
    ...
    hemm(T, MK, KN, MN, .{ .triangle = .k, .data = A }, B, C, .{
        .alpha = .{ .re = 1, .im = 0 },
        .beta = .{ .re = 0, .im = 0 },
    });
...
test "syrk real upper (triangle = second axis)" {
    ...
    // C[1,1] = 16+25+36 = 77
    syrk(T, NK, NN, A, .{ .triangle = .n2, .data = C }, .{ .alpha = 1.0, .beta = 0.0 });
...
test "syrk real lower (triangle = first axis)" {
    ...
    syrk(T, NK, NN, A, .{ .triangle = .n, .data = C }, .{ .alpha = 1.0, .beta = 0.0 });
...
test "syrk real nontrivial scalars" {
    ...
    syrk(T, NK, NN, A, .{ .triangle = .n2, .data = C }, .{ .alpha = 2.0, .beta = 3.0 });
...
test "herk complex upper" {
    ...
    //        = i*(-i) + (1-i)*(1+i)
    //        = 1 + 1+1 = 1 + 2 = 3 (real)
    herk(f64, NK, NN, A, .{ .triangle = .n2, .data = C }, .{ .alpha = 1.0, .beta = 0.0 });
...
test "herk complex lower" {
    ...
    // C[1,0] = 2*(1-i) = 2-2i
    // C[1,1] = 2*2 = 4
    herk(f64, NK, NN, A, .{ .triangle = .n, .data = C }, .{ .alpha = 1.0, .beta = 0.0 });
...
test "syr2k real upper (triangle = second axis)" {
    ...
    // C = [[34, 62], [62, 106]]
    syr2k(T, MK, NK, MN, A, B, .{ .triangle = .n, .data = C }, .{ .alpha = 1.0, .beta = 0.0 });
...
test "syr2k real nontrivial scalars" {
    ...
    syr2k(T, MK, NK, MN, A, B, .{ .triangle = .n, .data = C }, .{ .alpha = 2.0, .beta = 3.0 });
...
test "syr2k column-major" {
    ...
    syr2k(T, MK, NK, MN, A, B, .{ .triangle = .m, .data = C }, .{ .alpha = 1.0, .beta = 0.0 });
...
test "her2k complex upper" {
    ...
    her2k(f64, MK, NK, MN, A, B, .{ .triangle = .n, .data = C }, .{
        .alpha = .{ .re = 1, .im = 0 },
        .beta = 0.0,
    });
...
test "her2k column-major" {
    ...
    her2k(f64, MK, NK, MN, A, B, .{ .triangle = .m, .data = C }, .{
        .alpha = .{ .re = 1, .im = 0 },
        .beta = 0.0,
    });
...
test "trmm real left (triangle = second axis)" {
    ...
    trmm(T, MK, MN, .{ .triangle = .k, .data = A }, .non_unit, B, .{ .alpha = 1.0 });
...
test "trmm real right" {
    ...
    // Result: [[1, 2], [0, 3], [2, 13]]
    trmm(T, KN, MN, .{ .triangle = .n, .data = A }, .non_unit, B, .{ .alpha = 1.0 });
...
test "trmm real unit diagonal" {
    ...
    trmm(T, MK, MN, .{ .triangle = .k, .data = A }, .unit, B, .{ .alpha = 1.0 });
...
test "trmm complex" {
    ...
    trmm(T, MK, MN, .{ .triangle = .k, .data = A }, .non_unit, B, .{
        .alpha = .{ .re = 1, .im = 0 },
    });
...
test "trmm column-major" {
    ...
    trmm(T, MK, MN, .{ .triangle = .m, .data = A }, .non_unit, B, .{ .alpha = 1.0 });
...
test "trsm real left (triangle = second axis)" {
    ...
    // X = [[1], [3]]
    trsm(T, MK, MN, .{ .triangle = .k, .data = A }, .non_unit, B, .{ .alpha = 1.0 });
...
test "trsm real right" {
    ...
    trsm(T, KN, MN, .{ .triangle = .n, .data = A }, .non_unit, B, .{ .alpha = 1.0 });
...
test "trsm real unit diagonal" {
    ...
    // Solve [[1, 4], [0, 1]] * X = [[7], [2]]
    // x1 = 2, x0 = 7 - 4*2 = -1
    trsm(T, MK, MN, .{ .triangle = .k, .data = A }, .unit, B, .{ .alpha = 1.0 });
...
test "trsm complex" {
    ...
    trsm(T, MK, MN, .{ .triangle = .k, .data = A }, .non_unit, B, .{
        .alpha = .{ .re = 1, .im = 0 },
    });
...
test "trsm column-major" {
    ...
    trsm(T, MK, MN, .{ .triangle = .m, .data = A }, .non_unit, B, .{ .alpha = 1.0 });
...
test "trmm with alpha" {
    ...
    // 3 * [[2], [7]] = [[6], [21]]
    trmm(T, MK, MN, .{ .triangle = .m, .data = A }, .non_unit, B, .{ .alpha = 3.0 });
...
test "trsm with alpha" {
    ...
    trsm(T, MK, MN, .{ .triangle = .m, .data = A }, .non_unit, B, .{ .alpha = 2.0 });
